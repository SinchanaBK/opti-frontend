...
export const assetsApi = {
  ...
  create: (p: AssetCreate, token: string) =>
    apiFetch<Asset>("/assets/", { method: "POST", body: JSON.stringify(p) }, token),
  update: (id: number, p: AssetUpdate, token: string) =>
    apiFetch<Asset>(`/assets/${id}`, { method: "PUT", body: JSON.stringify(p) }, token),
  delete: (id: number, token: string) =>
    apiFetch<void>(`/assets/${id}`, { method: "DELETE" }, token),
};

export const usersApi = {
  getAll: (token: string) => apiFetch<User[]>("/users/", {}, token),
  create: (p: UserCreate, token: string) =>
    apiFetch<User>("/users/", { method: "POST", body: JSON.stringify(p) }, token),
  delete: (id: number, token: string) =>
